...
/* 
    ...
    - two team play against each other once.
    - Each team one match each round
    - Double -> playing 2 consecutive in (home | away)

    - No more than one double
    - No double on certain rounds
    - home (6 - 7) times
    - 
...
noDoubleRound(D):- noDoubles(L), member(D,L).
...
noDoubles:- 
    noDoubleRound(R), team(S), R0 is R-1,
    writeClause([  home(S,R0),  home(S,R) ]),
    writeClause([ -home(S,R0), -home(S,R) ]), fail.
noDoubles.
...
sixOrSevenHomes:-  
    team(S),
    findall( home(S,R), round(R), Lits), atLeast(6, Lits), atMost(7,Lits), fail.
sixOrSevenHomes.


/* doubles:-  % MUUUUY Ineficiente!!!
    team(S), otherTeam(S,T), otherTeam(S,T2), round(R), R0 is R-1, round(R0),
    writeClause([ -match(S,T,R0), -match(S,T2,R), double(S,R) ]), 
    writeClause([ -match(T,S,R0), -match(T2,S,R), double(S,R) ]), fail. 
doubles. */
...
displaySol(M):- nl, team(S), round(R), member(double(S,R), M ), write(double(S,R)), nl, fail.
