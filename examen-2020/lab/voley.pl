...
mustHome(S,R):- home(S,LR), member(R,LR).
...
%%%%%% NEW
maxUnsatisfied(Max):-
    findall( match(S,T,R), placed(S-T,R), Lits ),
...
define_hasDouble:-
    ...
    expressOr( hasDouble(S), Lits), fail.
define_hasDouble.

atMost12hasDouble:-
    findall( hasDouble(S), team(S), Lits ),
    atMost(12, Lits), fail.
atMost12hasDouble.
...
costOfThisSolution(M,Cost):- findall(S-T-R, (placed(S-T,R), \+member(match(S,T,R),M)),L), sort(L,L1), length(L1,Cost), !.
